        ...
        for row in tickers:
            ...
            try:
                ...
                if values:
                    ...
                    sr_history = values.pop("_sr_history", None)
                ...
                # BUG #10 FIX (19 Sep 2026): jeda kecil antar-ticker supaya
                # tidak membombardir Yahoo Finance tanpa henti (22 ticker x
                # ~4 request = 88+ request dalam hitungan detik sebelum fix
                # ini, memicu kegagalan diam-diam). Sudah dicatat sebagai
                # bug minor lama di 03_BUGS.md, dipatch bersamaan di sini.
                import time
                time.sleep(0.5)
...
def refresh_ihsg_trend():
    ...
    try:
        ...
        # DIAGNOSTIC LOGGING (19 Sep 2026): fungsi ini sebelumnya SELALU
        # "sukses tanpa exception" walau price/result gagal fetch (fail-silent
        # di level get_current_price/get_support_resistance), sehingga tidak
        # ada jejak SAMA SEKALI kenapa IHSG Signal jarang/tidak pernah
        # terbuat lewat scheduler asli. Log eksplisit di sini supaya ketahuan
        # di siklus jadwal berikutnya kalau ini penyebabnya.
        if price is None:
            frappe.log_error(
                "get_current_price('^JKSE') return None -- fetch gagal/timeout.",
                "FD-Trade IHSG Diagnostic"
            )
        if result is None:
            frappe.log_error(
                "get_support_resistance('^JKSE') return None -- fetch gagal/data histori tidak cukup.",
                "FD-Trade IHSG Diagnostic"
            )
